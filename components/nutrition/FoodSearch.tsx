    ...
    <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <input
          ...
          className="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-3 py-2.5 text-base text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none"
        />
        <button
          type="submit"
          disabled={searching}
          className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 text-white font-semibold px-4 py-2 rounded-lg text-sm transition"
        >
          {searching ? '…' : 'Search'}
        </button>
      </form>
      ...
      {hits && hits.length === 0 && (
        <p className="text-sm text-slate-400 mt-3">No foods found — try a simpler search term.</p>
      )}
            ...
            <div key={i}>
              ...
              >
                <span className="min-w-0 mr-2">
                  ...
                  {hit.brand && <span className="block text-xs text-slate-400 truncate">{hit.brand}</span>}
                </span>
                <span className="text-xs text-slate-300 shrink-0 text-right">
                  {hit.kcalPer100g} cal · {hit.proteinPer100g}g
                  <span className="block text-[10px] text-slate-500">per 100g</span>
                </span>
              </button>

              {selected === i && (
                <div className="flex items-center gap-2 bg-slate-800 rounded-lg px-3 py-2.5 mt-1 border border-slate-700">
                  <input
                    type="number"
                    inputMode="decimal"
                    value={grams}
                    onChange={(e) => setGrams(e.target.value)}
                    className="w-20 bg-slate-700 border border-slate-600 rounded-lg px-2 py-1.5 text-center text-sm font-bold text-white focus:border-blue-500 focus:outline-none"
